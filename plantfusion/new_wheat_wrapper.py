...
from elongwheat import (
    parameters as elongwheat_parameters,
    tillering as elongwheat_tillering,
)
...
class Wheat_wrapper(object):
    ...
    * senescwheat_facade
    ...
    def __init__(
        ...
        new_start_time = -1
        if run_from_outputs:
            previous_axes_outputs_dataframe = pandas.read_csv(os.path.join(out_folder, AXES_OUTPUTS_FILENAME))
            ...
            for inputs_filename in (
                ...
                self.inputs_dataframes[inputs_filename] = inputs_dataframe.where(inputs_dataframe.notnull(), None)
        ...
        management = pandas.read_csv(os.path.join(in_folder, MANAGEMENT_FILENAME), header=None, index_col=0).squeeze("columns")
        ...
        cnwheat_axes_initial_state = self.inputs_dataframes[AXES_INITIAL_STATE_FILENAME][
            [
                i
                for i in cnwheat_facade.cnwheat_converter.AXES_VARIABLES
                if i in self.inputs_dataframes[AXES_INITIAL_STATE_FILENAME].columns
            ]
        ].copy()
    ...
    def run(self, t_light):
        ...
        for t_senescwheat in range(t_light, t_light + self.SENESCWHEAT_TIMESTEP, self.SENESCWHEAT_TIMESTEP):
            ...
            for t_farquharwheat in range(
                ...
                Ta, Tsoil, ambient_CO2, RH, Ur = self.meteo.loc[
                    t_farquharwheat, ["air_temperature",'soil_temperature', "ambient_CO2", "humidity", "Wind"]
                ]
                ...
                for t_elongwheat in range(
                    ...
                    self.tillers_replications = elongwheat_tillering.update_tiller_replications(
                        ...
                        tillers_replications=self.tillers_replications,
                        gaic=self.GAIc,
    ...
    def end(self, simu_ran=True, run_postprocessing=False, run_graphs=False):
        ...
        if run_postprocessing:
            ...
            for postprocessing_file_basename, postprocessing_filename, index_columns in (
                ...
                postprocessing_df.to_csv(postprocessing_filepath, na_rep='NA', index=False, float_format='%.{}f'.format(PRECISION))
