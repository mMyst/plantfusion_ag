...
def simulation(in_folder, out_folder,start_wheat=None, simulation_length=2500, write_geo=False, geostep=1, run_postprocessing=False, run_graphs=False):
    try:
    ...
    N_fertilizations = {4176: 357143, 5088: 1000000} #22 février et 1er avril
    ...
    for t in range(wheat.start_time, wheat.start_time+simulation_length, wheat.SENESCWHEAT_TIMESTEP):
        if (t % light_timestep == 0) and (wheat.PARi_next_hours(t) > 0):
            ...
            if((write_geo==True) and (t%geostep*light_timestep == 0 )) : # save geometry as vtk file every geostep timestep
...
if __name__ == "__main__":
    ...
    out_folder = "outputs/cnwheat_default_meteo_lubbac"
    start_wheat = '17/12/2024'
    ...
    simulation(in_folder, out_folder,start_wheat, simulation_length, write_geo=write_geo, geostep=geostep, run_postprocessing=run_postprocessing,run_graphs=run_graphs)
