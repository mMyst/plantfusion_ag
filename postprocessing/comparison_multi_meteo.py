...
def photosynthesis(dict_current_axes, df_control_axes, pdf):
    # First graph: Total Photosynthesis
    ...
    # Second graph: Tiller Photosynthesis
    fig, axis = plt.subplots()

    df_control_axes['day'] = df_control_axes['t'] // 24 + 1
    axis.plot(df_control_axes['day'].unique(), df_control_axes.groupby('day')['Tillers_Photosynthesis'].sum(), label='Control', color='black', linestyle='--')

    for name, df in dict_current_axes.items():
        df['day'] = df['t'] // 24 + 1
        axis.plot(df['day'].unique(), df.groupby('day')['Tillers_Photosynthesis'].sum(), label=name)

    axis.set_xlabel('Time (day)')
    axis.set_ylabel('Tillers Photosynthesis (µmol C)')
    axis.legend()
...
if __name__ == '__main__':
    ...
    simulations_paths = {
        ...
        # 'Sim_lowN_0til_0N': r'C:\Users\agrumel\Documents\Données\Sorties CNWheat\lowN_monoculm\0.2m_0N\wheat',
        # 'Sim_lowN_4til_0N': r'C:\Users\agrumel\Documents\Données\Sorties CNWheat\lowN\0.2m_0N\wheat',
        # 'Sim_lowN_0til_450N': r'C:\Users\agrumel\Documents\Données\Sorties CNWheat\lowN_monoculm\0.2m_450N\wheat',
